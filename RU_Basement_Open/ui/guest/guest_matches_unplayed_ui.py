...
class UnplayedMatchesUI(MenuFrame):
	...
	def _get_all_unplayed_matches_in_correct_format(self):
		'''
		gets all unplayed matches from the logic layer and converts it into a format of a list
	...
	def display_menu(self, showing_page:int=0, list_of_all_match_results:list=[]):
		"""Display the menu screen for the unplayed matches"""
		LINE = "─"
	...
	def prompt_option(self, showing_page:int = 0):
		...
		while True:
			list_of_all_match_results = self._get_all_unplayed_matches_in_correct_format()
			self.clear_menu()
